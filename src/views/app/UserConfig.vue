...
import { PasswordInput } from "@/components/auth/authComponents";
  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error(
        data.error || "/user-config: Error al publicar la oferta",
      );
...
async function deleteUser() {
  const confirmed = window.confirm(
    "¿Deseas continuar con la eliminación de tu cuenta?\n ",
  );
  if (!confirmed) return;
  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error(
        data.error || "UserConfig: Error al publicar la oferta",
      );
    ...
    <SectionGroup>
      <SectionElement>
        <SectionHeader
          ...
          text="Datos principales de la cuenta"
      ...
      <SectionElement>
        <SectionHeader
          ...
          text="Acciones irreversibles para tu cuenta"
