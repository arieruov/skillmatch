...
onMounted(async () => {
	try {
		const response = await fetch(`${import.meta.env.VITE_API_URL}/api/job/getAllOffers`, {
			headers: {
...
<template>
	<ScreenLayout>
		<div class="mb-8">
			...
			<p className="text-slate-600">Explora todas las oportunidades disponibles</p>
		</div>

		<SectionGroup v-if="Array.isArray(jobsStore.jobsData) && jobsStore.jobsData.length > 0">
			<JobCard
