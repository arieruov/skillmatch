...
import { Building2, MapPin, DollarSign, Heart, ExternalLink } from 'lucide-vue-next';
...
async function saveOffer() {
	try {
		const response = await fetch(`${import.meta.env.VITE_API_URL}/api/job/saveOffer`, {
			method: 'POST',
			headers: {
		...
		if (!response.ok) {
			const error = new Error(data.error || '/job-details: Error al guardar la oferta');
...
onMounted(async () => {
	try {
		...
		if (!response.ok) {
			const error = new Error(data.error || '/job-details: Error al publicar la oferta');
...
<template>
	<ScreenLayout>
		<SectionGroup v-if="jobData">
			...
			>
				<h2 class="mb-4 text-2xl font-bold text-slate-900">Habilidades Requeridas</h2>
			...
			<section
				class="relative flex flex-col rounded-lg border border-slate-300 bg-slate-50 p-6 shadow-sm transition"
			>
				<h2 class="mb-4 text-2xl font-bold text-slate-900">Descripción de la oferta</h2>

				<div class="flex flex-col gap-4">
					<div>
						<h3 class="mb-2 text-lg font-bold text-slate-700">Sobre la empresa</h3>
					...
					<div>
						<h3 class="mb-2 text-lg font-bold text-slate-700">Responsabilidades</h3>
						<pre class="font-sans whitespace-pre-wrap text-slate-600">{{
