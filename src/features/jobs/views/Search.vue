...
import { useMatchJobsStore } from '@/features/jobs/stores/matchJobsStore';
...
async function getMatchOffers() {
	...
	try {
		const response = await fetch(`${import.meta.env.VITE_API_URL}/api/job/matchJobs`, {
			method: 'POST',
			headers: {
		...
		if (!response.ok) {
			const error = new Error(data.error || 'Search: Error al traer las ofertas');
			// @ts-ignore
...
<template>
	<ScreenLayout>
		<div class="mb-8">
			...
			<p class="text-slate-600">Encuentra ofertas que coincidan con tus habilidades</p>
		</div>
