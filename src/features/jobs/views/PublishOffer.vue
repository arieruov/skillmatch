...
async function publishOffer() {
	...
	const confirmed = window.confirm('¿Estás seguro de que deseas publicar esta oferta?\n ');

	if (!confirmed) return;

	try {
		const response = await fetch(`${import.meta.env.VITE_API_URL}/api/job/publishOffer`, {
			method: 'POST',
			headers: {
