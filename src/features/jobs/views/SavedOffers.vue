...
import { useSavedJobsStore } from '@/features/jobs/stores/savedJobsStore';
...
onMounted(async () => {
	try {
		const response = await fetch(`${import.meta.env.VITE_API_URL}/api/job/getAllSavedOffers`, {
			headers: {
		...
		if (!response.ok) {
			const error = new Error(data.error || '/saved-offers: Error al traer las ofertas');
			// @ts-ignore
