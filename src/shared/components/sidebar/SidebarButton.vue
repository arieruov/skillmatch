...
import { Home, Search, Bookmark, Briefcase, Leaf, SquarePen } from 'lucide-vue-next';

const props = defineProps<{
	icon: 'home' | 'search' | 'bookmark' | 'briefcase' | 'square_pen' | 'default';
	label: string;
	route: string;
	isActive: boolean;
}>();
...
<template>
	...
	>
		...
		/>
		<Bookmark
			v-else-if="props.icon === 'bookmark'"
			class="h-5 w-5 text-violet-600 transition-transform group-hover:scale-110"
		/>
		<Briefcase
			v-else-if="props.icon === 'briefcase'"
			class="h-5 w-5 text-violet-600 transition-transform group-hover:scale-110"
		/>
		<SquarePen
			v-else-if="props.icon === 'square_pen'"
			class="h-5 w-5 text-violet-600 transition-transform group-hover:scale-110"
		/>
		<Leaf
			v-else-if="props.icon === 'default'"
			class="h-5 w-5 text-violet-600 transition-transform group-hover:scale-110"
		/>
		<span class="font-medium text-slate-700 transition-colors group-hover:text-violet-700">
			{{ props.label }}
		</span>
	</button>
</template>
